...
class MultiplePapersInput(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "papers": [
                    {"id": "paper_1", "text": "Abstract: We introduce BERT..."},
                    {"id": "paper_2", "text": "Abstract: RoBERTa improves BERT..."},
                ]
            }
        }
...
async def extract_graph(payload: PaperInput):
    ...
    try:
        ...
        # Extract graph
        graph = await extractor.extract_from_paper(payload.text, payload.metadata or {})
...
async def extract_graphs(payload: MultiplePapersInput):
    ...
    try:
        ...
        if len(payload.papers) > 20:
            raise HTTPException(
                status_code=400, detail="Maximum 20 papers allowed per request"
            )
        ...
        response = {
            ...
            "summary": {
                ...
                "relationships_found": len(graph.get("relationships", [])),
                "unique_concepts": len(
                    [n for n in graph.get("nodes", []) if n.get("type") == "Concept"]
                ),
                "unique_methods": len(
                    [n for n in graph.get("nodes", []) if n.get("type") == "Method"]
                ),
