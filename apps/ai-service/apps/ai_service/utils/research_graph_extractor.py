...
class ResearchGraphExtractor:
    ...
    def _smart_truncate(self, text: str, max_chars: int) -> str:
        ...
        print(
            f"   📄 Smart truncated: {first_part_size} chars (front) + {last_part_size} chars (back)"
        )
    ...
    def _validate_graph(self, graph: dict[str, Any]) -> None:
        ...
        for node in graph.get("nodes", []):
            ...
            if "frequency" in props:
                assert isinstance(props["frequency"], int), (
                    f"frequency must be int: {node}"
                )
            if "importance" in props:
                assert props["importance"] in ["high", "medium", "low"], (
                    f"Invalid importance: {node}"
                )
        ...
        for rel in graph.get("relationships", []):
            ...
            assert isinstance(props, dict), (
                f"Relationship properties must be dict: {rel}"
            )

            # Validate property types if present
            if "confidence" in props:
                assert isinstance(props["confidence"], (int, float)), (
                    f"confidence must be number: {rel}"
                )
                assert 0.0 <= props["confidence"] <= 1.0, (
                    f"confidence must be 0-1: {rel}"
                )
    ...
    def _merge_graphs(self, graphs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for graph in graphs:
            for node in graph.get("nodes", []):
                ...
                if deduplication_key not in node_map:
                    ...
                    id_to_canonical_id_map[original_node_id] = existing_node[
                        "id"
                    ]  # Map current node's ID to the canonical ID

                    existing_node["properties"]["frequency"] += 1

                    # Update importance
                    current_importance = existing_node["properties"].get(
                        "importance", "medium"
                    )
                    new_importance = node.get("properties", {}).get(
                        "importance", "medium"
                    )
                    ...
                    if new_context and (
                        not existing_node["properties"].get("context")
                        or len(new_context)
                        > len(existing_node["properties"]["context"])
                    ):
                    ...
                    if combined_paper_ids:
                        existing_node["properties"]["paperId"] = ",".join(
                            sorted(list(combined_paper_ids))
                        )
                    else:
        ...
        for graph in graphs:
            for rel in graph.get("relationships", []):
                ...
                canonical_source_id = id_to_canonical_id_map.get(
                    rel["source"], rel["source"]
                )
                canonical_target_id = id_to_canonical_id_map.get(
                ...
                if rel_key not in relationship_set:
                    ...
                    merged_relationships.append(
                        {
                            ...
                            "type": rel["type"],
                            "properties": {
                                "confidence": rel.get("properties", {}).get(
                                    "confidence", 0.8
                                ),
                    ...
                    for existing_rel in merged_relationships:
                        if (
                            existing_rel["source"] == canonical_source_id
                            and existing_rel["type"] == rel["type"]
                            and existing_rel["target"] == canonical_target_id
                        ):
                            current_conf = existing_rel["properties"].get(
                                "confidence", 0.8
                            )
                            existing_rel["properties"]["confidence"] = min(
                                1.0, current_conf + 0.1
                            )
                            break
